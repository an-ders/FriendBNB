...
public struct Shimmer: ViewModifier {
	...
	public static let defaultAnimation = Animation.linear(duration: 0.75).delay(1).repeatForever(autoreverses: false)

	// A default gradient for the animated mask.
	public static let defaultGradient = Gradient(colors: [
		.white, // translucent
		.white.opacity(0.3), // opaque
		.white // translucent
	])
...
public extension View {
	...
	@ViewBuilder func shimmering(
		active: Bool = true, duration: Double, bounce: Bool = false, delay: Double = 1
	) -> some View {
		shimmering(
			active: active,
