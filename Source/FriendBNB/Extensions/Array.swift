...
extension Array where Element == Booking {
	...
	func current() -> [Booking] {
		self.filter { ($0.end > Date().minusDay()) }
	}
...
extension Array where Element == Availability {
	func dict() -> [String: [Availability]] {
		var dict: [String: [Availability]] = [:]
